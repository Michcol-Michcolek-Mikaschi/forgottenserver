...
class GesiorShop {
	...
	public static function getOfferImage($offer)
	{
		...
		if ($offer['type'] == 'item' || $offer['type'] == 'container') {
			$item_id = $offer['client_id'] ?? ($offer['item_id'] ?? null);
			if ($item_id) {
	...
	private static function parseOffer($_offer) {
		...
		switch($offer_type) {
			...
			case 'item':
				$offer = array_merge($offer, array(
					'item_id' => $offer_itemid1,
					'item_count' => $offer_count1,
					'client_id' => $offer_itemid2 > 0 ? (int)$offer_itemid2 : null
				));
				break;
